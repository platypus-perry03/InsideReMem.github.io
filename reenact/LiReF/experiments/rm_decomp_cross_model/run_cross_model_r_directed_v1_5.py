#!/usr/bin/env python3
"""Whole-depth R-directed component discovery and causal R/M-gap validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--design", type=Path, default=SCRIPT_DIR / "design_v1_5_r_directed_frozen.json")
...
def load_design(path: Path) -> dict[str, Any]:
    ...
    if design.get("study_id") != "cross_model_r_directed_homologue_v1_5":
    ...
    if design.get("layer_scope") != "all_transformer_blocks":
        raise RuntimeError("v1.5 must search every Transformer block")
    if design.get("candidate_selection") != "reasoning_mean_positive_and_positive_Delta_global_topk_no_depth_quota":
        raise RuntimeError("v1.5 must use the frozen R-directed writer rule")
...
def model_contract(model: Any) -> dict[str, Any]:
    c = model.config
    allowed = {"llama", "mistral", "olmo2", "gemma2"}
...
def component_frames(model: Any, layers: list[int], discovery: dict[str, Any], design: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for offset, layer_index in enumerate(layers):
        ...
        pre_group = pre_mean.reshape(2, heads, head_dim)
        ...
        head_group_mean = (pre_group * q[None, :, :]).sum(axis=2)
        head_delta = head_group_mean[GROUP_REASONING] - head_group_mean[GROUP_MEMORY]
        for index in range(heads):
            ...
            head_rows.append({
                ...
                "memory_mean_discovery": float(head_group_mean[GROUP_MEMORY, index]),
                "reasoning_mean_discovery": float(head_group_mean[GROUP_REASONING, index]),
        ...
        neuron_group_mean = z_mean * neuron_projection[None, :]
        ...
        for index in range(model.config.intermediate_size):
            ...
            neuron_rows.append({
                ...
                "memory_mean_discovery": float(neuron_group_mean[GROUP_MEMORY, index]),
                "reasoning_mean_discovery": float(neuron_group_mean[GROUP_REASONING, index]),
...
def select_candidates_and_controls(frames: list[pd.DataFrame], design: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for component_type in ("head", "neuron"):
        eligible = all_components[
            (all_components["component_type"] == component_type)
            & (all_components["reasoning_mean_discovery"] > 0)
            & (all_components["Delta_discovery"] > 0)
        ]
        chosen = eligible.sort_values(["Delta_discovery", "component_id"], ascending=[False, True]).head(topk).copy()
...
def validation_pass(
    ...
    selected["heldout_r_directed_same_sign"] = (
        (selected["reasoning_mean_validation"] > 0)
        & (selected["Delta_validation"] > 0)
        & (selected["reasoning_mean_discovery"] > 0)
        & (selected["Delta_discovery"] > 0)
    )
...
def intervention_pass(
    ...
    for candidate_number, candidate in enumerate(candidates["component_id"]):
        ...
        pass_checks = {
            "heldout_r_directed_same_sign": bool(v["heldout_r_directed_same_sign"]),
...
def run_model(args: argparse.Namespace, design: dict[str, Any], design_path: Path) -> None:
    ...
    validate_authorization(
        ...
        design_path,
        Path(__file__),
        SCRIPT_DIR / "STATIC_REVIEW_V1_5_R_DIRECTED.md",
        entry,
    )
    ...
    try:
        ...
        survivors = validation[
            validation["heldout_r_directed_same_sign"]
        ...
        summary = {
            ...
            "interpretation": "R-directed gap-supporting functional recurrence in this fixed R/M dataset and prompt only; not a reasoning circuit, behavioral cause, weight-level alignment, or a universal mechanism.",
            "hashes": {
...
def combined_report(design: dict[str, Any], design_path: Path) -> None:
    ...
    combined = {
        ...
        "class_recurrence_in_all_four_models": all_class,
        "meta_like_sparse_pattern_in_all_four_models": all_meta,
        "claim_boundary": "This study tests whole-depth functional recurrence of R-directed gap-supporting component classes. It does not identify a reasoning circuit, behavioral cause, exact cross-model correspondences, or the input feature that creates R/M separation.",
        "result_pdf_updated": False,
        "hashes": {"design": sha256_file(design_path)},
    }
    lines = [
        "# Cross-model R-directed Functional Homologue Search v1.5 결과", "", "상태: **COMPLETE**", "",
    ...
    lines.extend([
        "", "## 전체 판정", "",
        f"- 네 모델 모두에서 head+neuron class recurrence: **{'YES' if all_class else 'NO'}**",
        f"- 네 모델 모두에서 Meta-like 3-head+1-neuron sparse pattern: **{'YES' if all_meta else 'NO'}**",
        "", "## 해석 제한", "",
