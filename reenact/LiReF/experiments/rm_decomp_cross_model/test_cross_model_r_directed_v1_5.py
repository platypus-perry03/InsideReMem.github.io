"""Model-free tests for the frozen R-directed cross-model runner."""
...
import run_cross_model_r_directed_v1_5 as runner  # noqa: E402
...
class RDirectedCrossModelRunnerTests(unittest.TestCase):
    ...
    def test_candidate_selection_is_discovery_only_and_deterministic(self) -> None:
        ...
        for component_type, letter, width in (("head", "H", 20), ("neuron", "N", 30)):
            for index in range(width):
                rows.append({
                    ...
                    "memory_mean_discovery": -1.0,
                    "reasoning_mean_discovery": float(index + 2),
        ...
        self.assertTrue((candidates_a["reasoning_mean_discovery"] > 0).all())
    ...
    def test_candidate_selection_can_return_zero(self) -> None:
        frame = pd.DataFrame([{
            ...
            "memory_mean_discovery": -2.0, "reasoning_mean_discovery": -1.0,
    ...
    def test_frozen_design_prohibits_pdf_update(self) -> None:
        design = runner.load_design(HERE / "design_v1_5_r_directed_frozen.json")
        self.assertIs(design["result_pdf_update_allowed"], False)
        self.assertEqual(len(design["models"]), 4)
        self.assertEqual(
            design["candidate_selection"],
            "reasoning_mean_positive_and_positive_Delta_global_topk_no_depth_quota",
        )
