#!/usr/bin/env python3
"""Whole-depth M-directed component discovery and causal R/M-gap validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--design", type=Path, default=SCRIPT_DIR / "design_v1_3_m_directed_frozen.json")
...
def load_design(path: Path) -> dict[str, Any]:
    ...
    if design.get("study_id") != "cross_model_m_directed_homologue_v1_3":
    ...
    if design.get("layer_scope") != "all_transformer_blocks":
        raise RuntimeError("v1.3 must search every Transformer block")
    if design.get("candidate_selection") != "memory_mean_negative_and_positive_Delta_global_topk_no_depth_quota":
        raise RuntimeError("v1.3 must use the frozen M-directed writer rule")
...
def run_model(args: argparse.Namespace, design: dict[str, Any], design_path: Path) -> None:
    ...
    validate_authorization(
        ...
        design_path,
        Path(__file__),
        SCRIPT_DIR / "STATIC_REVIEW_V1_3_M_DIRECTED.md",
        entry,
    )
    ...
    try:
        ...
        summary = {
            ...
            "interpretation": "M-directed gap-supporting functional recurrence in this fixed R/M dataset and prompt only; not a memorization circuit, weight-level alignment, or a universal mechanism.",
            "hashes": {
...
def combined_report(design: dict[str, Any], design_path: Path) -> None:
    ...
    combined = {
        ...
        "claim_boundary": "This study tests whole-depth functional recurrence of M-directed gap-supporting component classes. It does not identify a memorization circuit, exact cross-model correspondences, or the input feature that creates R/M separation.",
        "result_pdf_updated": False,
        "hashes": {"design": sha256_file(design_path)},
    }
    lines = [
        "# Cross-model M-directed Functional Homologue Search v1.3 결과", "", "상태: **COMPLETE**", "",
