#!/usr/bin/env python3
"""Meta-Llama whole-depth M-directed component discovery and R/M-gap validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--design", type=Path, default=SCRIPT_DIR / "design_v1_4_meta_llama_m_directed_frozen.json")
...
def load_design(path: Path) -> dict[str, Any]:
    ...
    if design.get("study_id") != "meta_llama_m_directed_homologue_v1_4":
    ...
    if design.get("layer_scope") != "all_transformer_blocks":
        raise RuntimeError("v1.4 must search every Transformer block")
    if design.get("candidate_selection") != "memory_mean_negative_and_positive_Delta_global_topk_no_depth_quota":
        raise RuntimeError("v1.4 must use the frozen M-directed writer rule")
...
def model_contract(model: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "post_normalized_component_outputs": False,
    }
...
def select_candidates_and_controls(frames: list[pd.DataFrame], design: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for component_type in ("head", "neuron"):
        eligible = all_components[
            ...
            & (all_components["memory_mean_discovery"] < 0)
...
def validation_pass(
    ...
    selected["heldout_m_directed_same_sign"] = (
        (selected["memory_mean_validation"] < 0)
        & (selected["Delta_validation"] > 0)
        & (selected["memory_mean_discovery"] < 0)
...
def intervention_pass(
    ...
    for candidate_number, candidate in enumerate(candidates["component_id"]):
        ...
        pass_checks = {
            "heldout_m_directed_same_sign": bool(v["heldout_m_directed_same_sign"]),
...
def run_model(args: argparse.Namespace, design: dict[str, Any], design_path: Path) -> None:
    ...
    validate_authorization(
        ...
        design_path,
        Path(__file__),
        SCRIPT_DIR / "STATIC_REVIEW_V1_4_META_LLAMA_M_DIRECTED.md",
        entry,
    )
    ...
    try:
        ...
        survivors = validation[
            validation["heldout_m_directed_same_sign"]
        ...
        "interpretation": "Meta-Llama M-directed gap-supporting components in this fixed R/M dataset and prompt only; not a memorization circuit or behavioral cause.",
            "hashes": {
...
def combined_report(design: dict[str, Any], design_path: Path) -> None:
    ...
    combined = {
        ...
        "class_recurrence_in_authorized_models": all_class,
        "meta_like_sparse_pattern_in_authorized_models": all_meta,
        "claim_boundary": "This study tests Meta-Llama whole-depth M-directed gap-supporting components. It does not identify a memorization circuit, behavioral cause, or the input feature that creates R/M separation.",
        "result_pdf_updated": False,
        "hashes": {"design": sha256_file(design_path)},
    }
    lines = [
        "# Meta-Llama M-directed Functional Component Search v1.4 결과", "", "상태: **COMPLETE**", "",
    ...
    lines.extend([
        "", "## 전체 판정", "",
        f"- Meta-Llama에서 head+neuron class recurrence: **{'YES' if all_class else 'NO'}**",
        f"- Meta-like 3-head+1-neuron sparse pattern: **{'YES' if all_meta else 'NO'}**",
        "", "## 해석 제한", "",
        "- 같은 번호 또는 weight-level neuron alignment를 의미하지 않는다.",
        "- 모든 layer를 검색했고 Early/Middle/Late는 결과 표지일 뿐 후보 할당량이 아니다.",
        "- Meta-Llama contribution은 residual-addition direct decomposition이다.",
