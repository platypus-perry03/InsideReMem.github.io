#!/usr/bin/env python3
"""Whole-depth M-selective component discovery and causal R/M-gap validation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--design", type=Path, default=SCRIPT_DIR / "design_v1_2_m_selective_frozen.json")
...
def load_design(path: Path) -> dict[str, Any]:
    ...
    if design.get("study_id") != "cross_model_m_selective_homologue_v1_2":
    ...
    if design.get("layer_scope") != "all_transformer_blocks":
        raise RuntimeError("v1.2 must search every Transformer block")
    if design.get("candidate_selection") != "global_negative_Delta_topk_across_all_layers_no_depth_quota":
        raise RuntimeError("v1.2 must use the frozen M-selective negative-tail rule")
...
def select_candidates_and_controls(frames: list[pd.DataFrame], design: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for component_type in ("head", "neuron"):
        negative = all_components[(all_components["component_type"] == component_type) & (all_components["Delta_discovery"] < 0)]
        chosen = negative.sort_values(["Delta_discovery", "component_id"], ascending=[True, True]).head(topk).copy()
...
def validation_pass(
    ...
    selected["heldout_negative_same_sign"] = (selected["Delta_validation"] < 0) & (selected["Delta_discovery"] < 0)
...
def intervention_pass(
    ...
    for candidate_number, candidate in enumerate(candidates["component_id"]):
        ...
        pass_checks = {
            "heldout_negative_same_sign": bool(v["heldout_negative_same_sign"]),
...
def run_model(args: argparse.Namespace, design: dict[str, Any], design_path: Path) -> None:
    ...
    validate_authorization(
        ...
        design_path,
        Path(__file__),
        SCRIPT_DIR / "STATIC_REVIEW_V1_2_M_SELECTIVE.md",
        entry,
    )
    ...
    try:
        ...
        survivors = validation[
            validation["heldout_negative_same_sign"]
        ...
        summary = {
            ...
            "interpretation": "M-selective functional recurrence in this fixed R/M dataset and prompt only; not a memorization circuit, weight-level alignment, or a universal mechanism.",
            "hashes": {
...
def combined_report(design: dict[str, Any], design_path: Path) -> None:
    ...
    combined = {
        ...
        "claim_boundary": "This study tests whole-depth functional recurrence of M-selective component classes. It does not identify a memorization circuit, exact cross-model correspondences, or the input feature that creates R/M separation.",
        "result_pdf_updated": False,
        "hashes": {"design": sha256_file(design_path)},
    }
    lines = [
        "# Cross-model M-selective Functional Homologue Search v1.2 결과", "", "상태: **COMPLETE**", "",
